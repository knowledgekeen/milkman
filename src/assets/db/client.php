...
if($action == "addClient"){
	...
	$customertype_id=$data->customertype_id;
	$cno1 = $data->cno1;
...
if($action == "addCustomerType"){
	$headers = apache_request_headers();
	authenticate($headers);
    $data = json_decode(file_get_contents("php://input"));
	$customertypename=$data->custypename;
	$customertype=$data->customertype;
	if($_SERVER['REQUEST_METHOD']=='POST'){
        //Status: 1 == 'active'
		$sql = " INSERT INTO `customertype_register` (`customertypename`,`customertype`) values ('$customertypename','$customertype') " ;
		$result = $conn->query($sql);
		$customertype_id = $conn->insert_id;
		
	}
    $data1= array();
    if($result){
		$data1["status"] = 200;
		$data1["data"] = $customertype_id;
		header(' ', true, 200);
		//Logging
		$log  = "File: client.php - Method: $action".PHP_EOL.
		"Data: ".json_encode($data).PHP_EOL;
